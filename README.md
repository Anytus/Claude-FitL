...
| `archive/TestGame1/` … `archive/TestGame6/` | The finished games: saves, logs, journal, notes, reports, transcripts, post-mortem. Move a directory back under `games/` to replay it with `rollback`. |
...
```
python3 tools/ctl.py new-game TestGame7 --final-coup-only   # first time
python3 tools/ctl.py resume TestGame7       # after a container/session loss
