...
> **Harness changes before TestGame4.** Read these first; some of the
...
>   There is no card prompt for you to answer and `ctl.py send` refuses one.
...
>   unknown to everyone until drawn.
> - **Answering prompts.** `ctl.py seq "<expected>=><answer>" ...` sends
>   several answers in one call. Each is sent only if `<expected>` appears
>   in the current prompt, and `#<label>` answers are resolved to the menu
>   number at that moment. This is the fix for every menu-renumbering
>   mistake in section 1: a Limited Op that skips "Finished selecting", a
>   Special Activity menu that has dropped Advise, a final Train menu that
>   has dropped Pacify, all stop the sequence instead of eating a wrong
>   number. The rule "print the screen before every answer" is therefore
>   retired: it cost a screen dump and an extra call per prompt in
>   TestGame3, and `seq` performs the same check. Use `screen` only after a
>   `seq` has stopped. Bare prompts that want a typed name take a plain
>   answer (`Train in which space=>Saigon`); numbered ones take `#Saigon`.
...
>   to `reports/<game>/`. You never retype narration, and nothing needs
>   pasting "before the container can be lost": the logs are saved before
>   the program asks for anything.
> - **Board view.** `render.py` is brief by default (empty LoCs collapsed,
>   adjacency omitted); `render.py --full` shows everything. `diff.py` no
>   longer repeats the log lines (`--log` adds them).
...
> - **Program build 1.53+harness** (was 1.53+sbd): same save timing, plus
>   three adjacencies the release table had in one direction only are now
>   two-way, as rules 1.3.6 requires: Khanh Hoa with LOC Saigon -- An Loc --
>   Ban Me Thuot, Quang Duc-Long Khanh with LOC Cam Ranh -- Da Lat, and
>   LOC Kontum -- Dak To with LOC Da Nang -- Dak To. In TestGame1 to 3 a
>   March into the first space of each pair could not come from the second.
> - **Tool calls are logged** to `usage.log` by a hook; it changes nothing.
>
> Everything below about the program's prompts, operations, Coup phases and
> bot behaviour still holds.

Written for the next playing session, from three games: TestGame1 (lost to the
VC at the 3rd Coup, cards #63 to the Coup), TestGame2 (won at the 3rd Coup by
4, the full 1964 opening through 24 cards and three Coup rounds) and TestGame3
(won at the 3rd Coup by 8, the full 1964 opening through 39 cards, then
continued past the win for rules exploration). Everything here was observed at
...
**Still unobserved after three games:** Air Strike carried through to a hit
(the menu has been opened and aborted, never resolved); the Train "Transfer
patronage to ARVN resources" action, offered three times and executed none;
Sweep activation ratios outside Lowland; whether a US Patrol costs Resources;
the "add an ARVN Assault" option inside a US Assault, which has never been
prompted; Train's forced removal of ARVN cubes from the map when Available is
short of 6.
...
- **Some space prompts come with no list at all.** `Train in which space:`
  ...
  is no way to tell in advance. With `seq`, give the plain name and the
  controller types it; if the prompt turns out to be numbered, give
  `#<name>` instead. Either way a wrong form is rejected by the program,
...
- **Transfer patronage to ARVN resources.** Offered **only when Saigon is one
  ...
  Train of Saigon. **Still never executed in three games** — twice lost to a
  chained send, once declined in favour of a 12-point pacification — so its
  size is still unknown. Patronage is ARVN score, so this is the only US tool
  that lowers ARVN's number directly; test it the first time ARVN is the
  faction to beat.
...
- **Destinations are LoCs and Cities only, anywhere on the map.** Typing a
  Province was rejected with the full legal list: `'Quang Nam' is not valid.
  Must be one of: An Loc, Cam Ranh, Hue, Kontum, LOC Ban Me Thuot -- Da Lat,
  ... Qui Nhon, Saigon, or abort`. That list spanned the whole map, so cubes
  run along connected LoC routes rather than one space at a time.
- After `Finished moving cubes`: `US Patrol - Activating guerrillas on LOCs`,
  ...
  Police beside it — and removed an NVA Active Guerrilla.
...
- **`Assault a space with ARVN forces`** prints a numbered list of every space
  ...
  **Removing an insurgent Base pays +6 Aid on top of Advise's own +6**:
  `Each insurgent base removed adds +6 Aid`. This was the best single Advise of
  the game and it cost no Resources and no US piece.
...
- `Each insurgent base removed adds +6 Aid` fires from any removal, not just
  Advise's, and prints as its own line.
