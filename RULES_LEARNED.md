...
> **Harness note (unchanged since TestGame3).** Event
> cards are now drawn by the harness itself: `ctl.py advance` answers the
...
> 1968 piles, 12 events plus a Coup card each), so unlike TestGame1 and
> TestGame2, where Kevin dealt from a physical deck, which cards appear is
> unknown to everyone until drawn. `render.py` now shows each card's period
> and the on-deck card's position in its pile. Everything below about the
> program's prompts, operations, Coup phases and bot behaviour still holds.
